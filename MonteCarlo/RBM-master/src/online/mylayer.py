...
class Mylayer(Layer):
    ...
    def __init__(self, output_dim=1, hidden_dim=16, init='glorot_uniform', activation='linear', weights=None,
                 W_regularizer=None, b_regularizer=None, a_regularizer=None, 
                 activity_regularizer=None,
                 W_constraint=None, b_constraint=None, a_constraint=None,
    ...
    def build(self, input_shape):
        ...
        if self.bias:
            ...
            self.a = K.zeros((input_dim,),name='{}_a'.format(self.name))
            self.c = K.zeros((1,),name='{}_c'.format(self.name))
            self.trainable_weights = [self.W, self.b, self.a, self.c]
        else:
            self.b = K.zeros((self.hidden_dim,),name='{}_b'.format(self.name))
            self.c = K.zeros((1,),name='{}_c'.format(self.name))
        ...
        if self.bias and self.a_regularizer:
            self.a_regularizer.set_param(self.a)
            self.regularizers.append(self.a_regularizer)
        ...
        if self.bias and self.a_constraint:
            self.constraints[self.a] = self.a_constraint
    ...
    def call(self, x, mask=None):
        ...
        if self.bias:
            output += K.dot(x, self.a)
        return self.activation(output) + self.c 
