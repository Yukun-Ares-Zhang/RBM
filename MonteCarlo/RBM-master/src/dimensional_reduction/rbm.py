...
python rbm.py -f ../data/copising_L40.configs  -n_visible=1600 -n_hidden 4  -training_epochs=50 -batch_size=100
"""
...
def test_rbm(learning_rate=0.1, training_epochs=15,
             ...
             n_chains=10, n_samples=5, output_folder='../data/rbm_plots',
             n_visible = 784, n_hidden=500, filename='../data/isingconfig_T2.5.dat'):
    """
    ...
    X_train, y_train, X_test, y_test = load_mydata(filename) 
    ...
    L = int(numpy.sqrt(n_visible))
    M = int(numpy.sqrt(n_hidden))
    ...
    image_data = numpy.zeros(
        ((L+1) * n_samples + 1, (L+1) * n_chains - 1),
        dtype='uint8'
    )
    for idx in range(n_samples):
        ...
        image_data[(L+1) * idx:(L+1) * idx + L, :] = tile_raster_images(
            X=vis_mf,
            img_shape=(L, L),
...
if __name__ == '__main__':
    ...
    test_rbm(training_epochs = args.training_epochs, 
