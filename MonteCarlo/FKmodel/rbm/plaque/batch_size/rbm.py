...
if __name__=='__main__':
    ...
    for batch_size in [ 5, 50, 500, 5000 ]:
        reftime = time.time()
        if os.path.exists("data")==False:
            os.makedirs("data")
        os.chdir("data")
        if os.path.exists("batch_size="+str(batch_size))==False:
            os.makedirs("batch_size="+str(batch_size))
        os.chdir("batch_size="+str(batch_size))
        ...
        for n_iteration in [10, 100, 1000]: 
            ...
            rbm = BernoulliRBM(n_components=100, learning_rate=lr, batch_size=batch_size, n_iter=n_iteration, random_state=42, verbose=0)
            rbm.fit(Training_v)
        ...
        fig.suptitle("batch_size="+str(batch_size))
        plt.savefig("batch_size="+str(batch_size)+".png")
        os.chdir("..")
        
        print("finished batch_size="+str(batch_size)+" in %5.2f min, Total time: %5.2f min"%((time.time()-reftime)/60,(time.time()-total_reftime)/60))
