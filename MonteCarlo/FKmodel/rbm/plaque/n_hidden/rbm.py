...
if __name__=='__main__':
    ...
    mask = np.isclose(Training_v, -1, atol=1e-8)
    Training_v[mask] = 0
    mask = np.isclose(Test_v, -1, atol=1e-8)
    Test_v[mask] = 0
    
    lr = 0.001  
    batch_size = 10
    
    for n_components in [ 10, 100, 1000, 10000 ]:
        reftime = time.time()
        if os.path.exists("data")==False:
            os.makedirs("data")
        os.chdir("data")
        if os.path.exists("n_components="+str(n_components))==False:
            os.makedirs("n_components="+str(n_components))
        os.chdir("n_components="+str(n_components))
        ...
        for n_iteration in [10, 100, 1000]: 
            ...
            rbm = BernoulliRBM(n_components=n_components, learning_rate=lr, batch_size=batch_size, n_iter=n_iteration, random_state=42, verbose=0)
            rbm.fit(Training_v)
        ...
        fig.suptitle("n_components="+str(n_components))
        plt.savefig("n_components="+str(n_components)+".png")
        os.chdir("..")
        
        print("finished n_components="+str(n_components)+" in %5.2f min, Total time: %5.2f min"%((time.time()-reftime)/60,(time.time()-total_reftime)/60))